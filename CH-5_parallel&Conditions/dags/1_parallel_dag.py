...
def parallel_dag():
    ...
    @task.python
    def fetch_db():
        data = {"type": "db", "daata": ["data4", "data5", "data6"]}
        return data
    ...
    db_data = fetch_db()
    s3_data = fetch_s3()
    process_data_instance = process_data(api_data, db_data, s3_data)

    # task_bash runs first, then the three fetch tasks run in parallel, then process_data
